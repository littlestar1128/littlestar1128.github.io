...
- 多个标签要写成 YAML 列表，不要写成 `嵌入式, 问题排查`。
- `categories` 使用列表格式，每篇文章选一个主要分类。
- 文件保存编码应为 UTF-8。
- `---` 必须成对出现。
...
| 分类 | 适合的内容 |
| --- | --- |
| 技术笔记 | 嵌入式、FPGA、界面开发、技术原理和问题排查 |
| 工具与实践 | Git、博客搭建、AI 自动化、工具使用和资源整理 |
| 生活记录 | 日常生活、校园记录、猫咪、旅行和阅读感想 |

分类按文章的主要内容选择，标签补充具体技术和主题，每篇建议使用 2–4 个标签。暂不设置二级分类，后续文章增多时再细分。
